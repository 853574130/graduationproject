...
<template>
  <div>
    ...
    <el-table ref="multipleTable" :data="articles" tooltip-effect="dark" style="width: 100%;overflow-x: hidden; overflow-y: hidden;" max-height="390" @selection-change="handleSelectionChange" v-loading="loading">
      ...
      <el-table-column prop="userId" label="作者" width="120" align="left">
      </el-table-column>
...
//  var bus = new Vue()
  ...
  export default{
    ...
    methods: {
      ...
      //       url = "/article/dustbin";
      //     }
      //     putRequest(url, {aids: this.dustbinData, state: state}).then(resp=> {
      //       if (resp.status == 200) {
      //         var data = resp.data;
    ...
    props: ['showEdit', 'showDelete', 'activeName','state'] 
}
</script>
